...
class TricentisReporter implements Reporter {
  ...
  constructor() {
    ...
    cycleName = process.env.CYCLE || (process.env.PROJECT_NAME + new Date().toLocaleString());
  ...
  async onEnd(result: FullResult) {
    const createTestRunUrl = `https://api.ttm4j.tricentis.com/v1/projects/${process.env.PROJECT_KEY}/test-runs`;
    ...
    await axios
      .post(createTestRunUrl, this.report, {
        headers: {
          ...
          Authorization: process.env.JIRA_KEY,
        },
      })
      .then((response) => {
        console.log(
