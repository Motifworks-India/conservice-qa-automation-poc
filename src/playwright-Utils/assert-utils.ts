...
import { ExpectOptions, ExpectTextOptions, SoftOption } from '../../config/ui/optional-parameter-types';
...
export async function assertSaveButton(
  ...
  await expectElementToHaveCss(input, 'color', 'rgb(10, 3, 55)', options);
...
export async function assertCancelButton(
  ...
  if (hoverCheck) {
    await hover(input);
    await expectElementToHaveCss(input, 'background-color', 'rgb(10, 3, 55)', options);
